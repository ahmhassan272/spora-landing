...
import { Trophy, Calendar, Star, MapPin, Clock } from 'lucide-react';
import { useLanguage } from '@/context/LanguageContext';

/* ─── GROUP STANDINGS DATA ─── */

const GROUP_A = [
    { team: 'Mangoes', p: 3, w: 2, d: 1, l: 0, gd: 14, pts: 7, cup: 'gold' as const },
    { team: 'Haram Ball', p: 3, w: 2, d: 0, l: 1, gd: -1, pts: 6, cup: 'gold' as const },
    { team: 'Bonita Banana SC', p: 3, w: 1, d: 0, l: 2, gd: -10, pts: 3, cup: 'silver' as const },
    { team: 'Palestina', p: 3, w: 0, d: 1, l: 2, gd: -3, pts: 1, cup: 'silver' as const },
];

const GROUP_B = [
    { team: 'Ashawo FC', p: 3, w: 2, d: 1, l: 0, gd: 4, pts: 7, cup: 'gold' as const },
    { team: 'Ballers United', p: 3, w: 2, d: 0, l: 1, gd: 2, pts: 6, cup: 'gold' as const },
    { team: 'Ay Caramba', p: 3, w: 1, d: 0, l: 2, gd: 2, pts: 3, cup: 'silver' as const },
    { team: 'HIK FC', p: 3, w: 0, d: 1, l: 2, gd: -8, pts: 1, cup: 'silver' as const },
];

/* ─── TYPES ─── */

type TeamRow = typeof GROUP_A[number];
...
const SATURDAY_FIXTURES: FixtureRow[] = [
    { time: '18:00 – 18:40', labelKey: 'semiFinal1', homeLiteral: 'Bonita Banana SC', awayLiteral: 'HIK FC' },
    { time: '18:50 – 19:30', labelKey: 'semiFinal2', homeLiteral: 'Ay Caramba', awayLiteral: 'Palestina' },
    { time: '19:40 – 20:20', labelKey: 'thirdPlace', homeKey: 'loserSF1', awayKey: 'loserSF2' },
    { time: '20:30 – 21:10', labelKey: 'silverFinal', homeKey: 'winnerSF1', awayKey: 'winnerSF2' },
];

const SUNDAY_FIXTURES: FixtureRow[] = [
    { time: '18:30 – 19:10', labelKey: 'semiFinal1', homeLiteral: 'Mangoes', awayLiteral: 'Ballers United' },
    { time: '19:20 – 20:00', labelKey: 'semiFinal2', homeLiteral: 'Ashawo FC', awayLiteral: 'Haram Ball' },
    { time: '20:10 – 20:50', labelKey: 'thirdPlace', homeKey: 'loserSF1', awayKey: 'loserSF2' },
    { time: '21:00 – 21:40', labelKey: 'goldFinal', homeKey: 'winnerSF1', awayKey: 'winnerSF2' },
];

/* ─── SUB-COMPONENTS ─── */

function StandingsTable({ title, teams, teamLabel }: { title: string; teams: TeamRow[]; teamLabel: string }) {
    return (
        <div>
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="bg-spora/20 text-spora">
                            {[teamLabel, 'P', 'W', 'D', 'L', 'GD', 'Pts'].map((col, ci) => (
                                <th key={ci} className={`py-3 px-3 font-semibold uppercase tracking-wider text-xs ${ci === 0 ? 'text-left' : 'text-center'}`}>
                                    {col}
                                </th>
                            ))}
                        </tr>
...
function CupBadge({ cup }: { cup: 'gold' | 'silver' }) {
    const { t } = useLanguage();
    const label = cup === 'gold' ? t('tournament', 'badgeGold') : t('tournament', 'badgeSilver');
    return (
        <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
            cup === 'gold' ? 'bg-amber-400/15 text-amber-400' : 'bg-gray-500/15 text-gray-400'
        }`}>
            {cup === 'gold' ? '🥇' : '🥈'} {label}
        </span>
    );
...
function FixtureCardRow({ fixture }: { fixture: FixtureRow }) {
    const { t } = useLanguage();
    const label = t('tournament', fixture.labelKey);
    const home = fixture.homeLiteral ?? t('tournament', fixture.homeKey!);
    const away = fixture.awayLiteral ?? t('tournament', fixture.awayKey!);
    const isFinal = fixture.labelKey.includes('Final') && !fixture.labelKey.includes('semi');
    ...
    return (
        <div className={`flex items-center gap-3 p-4 rounded-xl border transition-colors ${
            ...
            <div className="shrink-0 w-[6.5rem] text-center">
                <span className="text-xs font-bold text-spora tabular-nums">{fixture.time}</span>
            </div>
            ...
            <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                >
                    ...
                    <div className="inline-flex items-center gap-2.5 px-5 py-2 rounded-full bg-spora/10 border border-spora/30 text-sm font-bold mb-5">
                        ...
                        <span className="text-spora">{t('tournament', 'liveHub')}</span>
                    </div>
                ...
                <motion.div
                    ...
                    className="mb-14"
                >
                    <h3 className="text-xl sm:text-2xl font-bold text-white mb-2 text-center">{t('tournament', 'standingsTitle')}</h3>
                    <p className="text-sm text-text-muted text-center mb-6">{t('tournament', 'standingsSubtitle')}</p>
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <StandingsTable title={t('tournament', 'groupA')} teams={GROUP_A} teamLabel={t('tournament', 'colTeam')} />
                        <StandingsTable title={t('tournament', 'groupB')} teams={GROUP_B} teamLabel={t('tournament', 'colTeam')} />
                    </div>
                </motion.div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div className="rounded-2xl bg-card border border-gray-500/30 p-5 sm:p-6">
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <h4 className="font-bold text-white text-lg">{t('tournament', 'saturdayTitle')}</h4>
                            </div>
                            <div className="flex items-center gap-4 text-xs text-text-muted mb-5">
                                ...
                                <span className="flex items-center gap-1"><Clock size={12} /> {t('tournament', 'arrival')}: 17:45</span>
                            </div>
                            <div className="space-y-2.5">
                                {SATURDAY_FIXTURES.map((f, i) => (
                                    <FixtureCardRow key={i} fixture={f} />
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="rounded-2xl bg-card border border-amber-500/30 p-5 sm:p-6">
                            <div className="flex items-center gap-2 mb-1">
                                <span className="text-lg">🥇</span>
                                <h4 className="font-bold text-white text-lg">{t('tournament', 'sundayTitle')}</h4>
                            </div>
                            <div className="flex items-center gap-4 text-xs text-text-muted mb-5">
                                <span className="flex items-center gap-1"><MapPin size={12} /> BeStrong Pallag</span>
                                <span className="flex items-center gap-1"><Clock size={12} /> {t('tournament', 'arrival')}: 18:15</span>
                            </div>
                            <div className="space-y-2.5">
